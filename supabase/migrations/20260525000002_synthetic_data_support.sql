-- v0.1.3 — Synthetic-data support for pattern-clustering prototyping.
--
-- Add `is_synthetic` flag to `make_scenarios` so generator-produced rows can coexist
-- with real ingested rows without polluting retrieval. Default exclusion is baked
-- into search_scenarios so chat + search never cite synthetic rows.
--
-- Purge synthetic rows in one statement: DELETE FROM make_scenarios WHERE is_synthetic;

ALTER TABLE public.make_scenarios
  ADD COLUMN IF NOT EXISTS is_synthetic boolean NOT NULL DEFAULT false;

-- Index for fast filtering — most queries are `is_synthetic = false`.
CREATE INDEX IF NOT EXISTS make_scenarios_is_synthetic_idx
  ON public.make_scenarios (is_synthetic);

-- Bump search_scenarios to take a new `p_include_synthetic` flag, default false.
-- Existing callers (the wrapper in lib/retrieval/hybrid-search.ts) keep working
-- without changes — they just get real-only results.
...
AS $$
  ...
  candidates AS (
    ...
    WHERE (af.apps_array     IS NULL OR s.apps_involved ?| af.apps_array)
      ...
      AND (p_include_synthetic OR s.is_synthetic = false)
